...
def extract_match_info(url):
    ...
    if response.status_code != 200:
        print(f"Failed to fetch match-info: {info_url}")
    ...
    winner_element = soup.find("div", string=re.compile("won", re.IGNORECASE))
    winner = winner_element.text.strip() if winner_element else None

    # Toss (safe)
    toss_element = soup.find("td", string="Toss")
    toss = toss_element.find_next("td").text.strip() if toss_element else None
    
    toss_win, toss_decision = None, None
    if toss:
        try:
            if "&" in toss:
                toss_win, toss_decision = toss.split(" & ")
                toss_win = shorten_team_name(toss_win.replace("toss", "").replace("won", "").strip())
                toss_decision = toss_decision.replace("chose to", "").strip().lower()
            elif "opted to" in toss:
                parts = toss.split("opted to")
                toss_win = shorten_team_name(parts[0].strip())
                toss_decision = parts[1].strip().lower()
            elif "elected to" in toss:
                parts = toss.split("elected to")
                toss_win = shorten_team_name(parts[0].strip())
                toss_decision = parts[1].strip().lower()
                 
            else:
                toss_win = shorten_team_name(toss.split()[0].strip())
        except Exception as e:
            print(f"Error processing toss info: {e}")
    
    # Venue (safe)
    venue_element = soup.find("td", string="Venue")
    venue = venue_element.find_next("td").text.strip() if venue_element else None
    if venue:
        try:
            parts = venue.split(",")
            venue = parts[-2].strip() if len(parts) >= 2 else parts[0].strip()
        except Exception as e:
            print(f"Error processing venue info: {e}")
    
    return winner, toss_win, toss_decision, toss, venue
...
def extract_cricket_data(url,Match_Details):
    ...
    winner, toss_win, toss_decision, toss, venue = extract_match_info(url)
    ...
    if response.status_code == 200:
            ...
            if accordion:
                ...
                if table_data:
                    ...
                    if df.shape[1] >= 5:
                        ...
                        df['Odds'] = df['Odds'].replace({'\xa0': ' ', '\r\n': '', '\n': ''}, regex=True).str.strip()
                        df[['Odd_A', 'Odd_B']] = df['Odds'].str.split(' ', expand=True, n=1)
                        df['Odds'] = df['Odds'].str.replace(r'\s+', ' ', regex=True).str.strip()
        ...
        os.makedirs('/home/umukesh/Desktop/cricket/ipl/data/raw/2025/innings1', exist_ok=True)
        os.makedirs('/home/umukesh/Desktop/cricket/ipl/data/raw/2025/innings2', exist_ok=True)
        os.makedirs('/home/umukesh/Desktop/cricket/ipl/data/processed/2025', exist_ok=True)

        # Construct full file paths for the directories
        innings1_filename = f'innings1_{name}_2025.csv'
        innings2_filename = f'innings2_{name}_2025.csv'
        result_filename = f'data_{name}_2025.csv'

        innings1_filepath = f'/home/umukesh/Desktop/cricket/ipl/data/raw/2025/innings1/{innings1_filename}'
        innings2_filepath = f'/home/umukesh/Desktop/cricket/ipl/data/raw/2025/innings2/{innings2_filename}'
        result_filepath = f'/home/umukesh/Desktop/cricket/ipl/data/raw/2025/match/{result_filename}'
        ...
        missing_data_path = '/home/umukesh/Desktop/cricket/ipl/data/raw/missing_data.json'
    ...
    # Check if the missing_data.json file exists
        ...
        if name not in missing_data['2025']:
            missing_data['2025'].append(name)
...
# url = "https://www.cricketmazza.com/match/match-info/indian-premier-league-2025-kolkata-knight-riders-royal-challengers-bengaluru-ipl--1st"
# innings1, innings2, result = extract_cricket_data(url)


df = pd.read_csv('/home/umukesh/Desktop/cricket/ipl/data/raw/URL/ipl2025.csv')
match_url_list = df['Match URL'][55:].tolist()
Match_Details=df['Match Details'][55:].tolist()
